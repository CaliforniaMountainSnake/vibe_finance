...
import { ticker } from './dbRepositoryTestHelpers'

export function assertDbRepositorySettings(makeRepo: () => DbRepositoryInterface) {
  ...
  describe('setSetting / getSetting', () => {
    ...
    it('persists null value', async () => {
      await repo.setSetting('totalBaseTicker', ticker('binance', 'btc'))
      await repo.setSetting('totalBaseTicker', null)
