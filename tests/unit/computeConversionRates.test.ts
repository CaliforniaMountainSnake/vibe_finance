...
import { makeRate } from '../helpers/dbRepositoryTestHelpers'
...
describe('computeConversionRates (с реальным DexieRepository + fake-indexeddb)', () => {
  ...
  it('возвращает курс для каждого холдинга', async () => {
    ...
    expect(result['h1']).toBeCloseTo(76808.44, 2) // btc→usdt
    expect(result['h2']).toBeCloseTo(76808.44 / 36.379, 3) // eth→usdt = usdt.btcPrice / eth.btcPrice
  })
  ...
  it('кросс-сорс: холдинг из binance, итог из coingecko', async () => {
    ...
    expect(result['cross']).toBeCloseTo(76808.44, 2)
  })
